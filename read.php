...
<?php
	...
	if(isset($_POST["sort"])){
		...
		$sort="order by cast(kakusu as signed)".$_POST["sort"];
		}
	}else{
		$sort="";
	}
...
if($key!=""){
	if($bu!=""){
		...
		$dbbu.=$bu;
		$dbbu.="%";
				if ($stmt = mysqli_prepare($Link, "SELECT KanjiID,KanjiPass,KanjiRubi,KanjiMean,Rei,Kakusu,Bushu,PointX,PointY FROM newkanji WHERE KanjiRubi LIKE ? AND Bushu LIKE ? or KanjiMean LIKE ? AND Bushu LIKE ? or Rei LIKE ? AND Bushu LIKE ? ".$sort."")) {
				mysqli_stmt_bind_param($stmt, "ssssss", $dbkeyw,$dbbu,$dbkeyw,$dbbu,$dbkeyw,$dbbu);
			...
			<form action="" id="kanjiSearch" method="post" name="form">
				...
				<div class="form__group">
					<label class="form__group__label">画数で並び替え</label>
						<label for="sort-normal" class="embossed_btn form__group__check">標準<input type="radio" id="sort-normal" class="form__group__radio" name="sort" value="" checked="checked"></label>

<!--					<a href="" onclick="document.form1.submit();return false;">-->
						<label for="sort-up" class="embossed_btn form__group__check">昇順<input type="radio" id="sort-up" class="form__group__radio" name="sort" value="ASC"></label>

<!--					</a>-->
<!--					<a href="" onclick="document.form2.submit();return false;">-->
						<label for="sort-down" class="embossed_btn form__group__check">降順<input type="radio" id="sort-down" class="form__group__radio" name="sort" value="DESC"></label>
