...
$(function() {
	...
	if(document.URL.match('/read')) {
		...
		let animationArray = ['kanji_rotate', 'kanji_topdown', 'kanji_rotate360', 'kanji_downtop', 'kanji_scaleout'];
		$('.panel').each(function() {
		...
		// });
        // ソートのチェックの状態表示
		$('.form__group__check').on('click', function() {
			$('.form__group__check').removeClass('checked');
			console.log('ago');
			let radioCheck = $(this).find('.form__group__radio').prop('checked');
			if(radioCheck == true) {
				console.log('true'+radioCheck);
				$(this).addClass('checked');
			} else {
				$(this).removeClass('checked');
				console.log('false'+radioCheck);
			}
		});
	}
	...
	// option.php用の処理
    // プレゼン審査用
	if(document.URL.match('/option')) {
		console.log('main.js : option -> ');
		$('.form__group__input').each(function(i) {
			console.log('main.js : form__group__input each -> ');
	...
	$('.form__group__input').on('change', function() {
		console.log('main.js : form__group__input on change -> ');
		let target = $(this).attr('name');
		if($(this).val() != '') {
			$('.form__group__label').each(function(i, elem) {
				if($(this).hasClass(target)) {
